...
public class GraderV2 {
	...
	private int testNumVertices(int testIdx)
	{
		...
		try {
			...
			if (searchEngine.internet.vertexList.size() == 6)
				...
				System.out.println("number vertices added: "+searchEngine.internet.vertexList.size());
				grade = 0;
			}
	...
	private int testNumEdges(int testIdx)	//Test num of edges added during crawl and index. Uses testAcyclic.xml
	{
		...
		try {
			...
			if(numEdges == 21)
				...
				System.out.println("number edges found: "+numEdges);
				grade = 0;
			}
			write(gradeString(grade, maxScore, comment));
		}
	...
	private int testAssignRanks(int testIdx)
	{
		...
		try {
			...
			searchEngine.assignPageRanks(.01);
			for (String v : vertices) {
	...
	private int testComputeRanks(int testIdx)
	{
		...
		try {
			...
			expectedResults.put("www.cs.mcgill.ca", 1.125);
			expectedResults.put("www.ea.com", 1.225);
			expectedResults.put("www.ubisoft.com", 0.9333333);
			expectedResults.put("www.eidos.com", 0.766666667);
			expectedResults.put("www.unreal.com", 0.725);
			expectedResults.put("www.unity.com", 1.225);
	...
	private int testGetResults0(int testIdx)		//Test the final result. Uses testAcyclic.xml
	{
		...
		try {
			...
			ArrayList<String> results = searchEngine.getResults("Enix");	//try a word in the graph

			if(results != null && results.get(0).equals("www.eidos.com")==true){	//Make sure the most relevent url is correct
				grade=5;
			}
			else{
...
//                long durationFast = TimeUnit.NANOSECONDS.toMicros(endTime-startTime);
                ...
                if ((durationSlow/durationFast) >= 0) {
